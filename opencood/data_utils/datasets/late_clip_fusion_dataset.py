...
# This is the implementation of the LateClipFusionDataset class for contrastive learning stage2.
...
def getLateClipFusionDataset(cls):
    """
    cls: the Basedataset.
    """
    class LateClipFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            self.ego_modality = params['heter']['ego_modality'] # "m1" or "m3" depending on which modality is the student

            self.modality_name_list = list(params['heter']['modality_setting'].keys())
            self.sensor_type_dict = OrderedDict()
            
            self.lidar_channels_dict = params['heter'].get('lidar_channels_dict', OrderedDict())
        ...
        def __getitem__(self, idx):
            """
                e.g. base_data_dict.keys() = ["650", "659"] corresponds to the 2 CAVs' id in the scene,
                where one of them has attribute 'ego' = True. 
            """
            base_data_dict = self.retrieve_base_data(idx)

            # determine the data format by the train flag
            if self.train:
        ...
        def get_item_train(self, base_data_dict):
            ...
            selected_cav_processed = self.get_item_train_single_car(selected_cav_base)
        ...
        def get_item_test(self, base_data_dict, idx):
            ...
            for cav_id in cav_id_list:
                ...
                selected_cav_processed = \
                    self.get_item_train_single_car(selected_cav_base)
        ...
        def get_item_train_single_car(self, selected_cav_base):
            """
            Process a single CAV's information for the train/test pipeline.
            ...
            selected_cav_processed : dict
                ...
                {
                    "processed_features_m*", (if requested for lidar)
                    "image_inputs_m*", (if requested for camera)
                    "origin_lidar",
                    "object_bbx_center",
                    "object_bbx_mask",
                    "object_ids",
            ...
            """
            selected_cav_processed = {}
            # modality_name = selected_cav_base['modality_name'] # assigned modality names, for example, "m1"
            for modality_name in self.modality_name_list: # for example, ["m1", "m3"]
                sensor_type = self.sensor_type_dict[modality_name] # either 'lidar' or 'camera'

                # label generated for student modality only
                ...
                # lidar
                if sensor_type == "lidar" or self.visualize:
                    suffix = str(self.lidar_channels_dict.get(modality_name, None))
                    lidar_np = selected_cav_base['lidar_np'][suffix]
                    lidar_np = shuffle_points(lidar_np)
                    lidar_np = mask_points_by_range(lidar_np, self.params['preprocess']['cav_lidar_range'])
                    lidar_np = mask_ego_points(lidar_np) # remove points that hit ego vehicle

                    # data augmentation, seems very important for single agent training, because lack of data diversity.
                    # only work for lidar modality in training.
                    if not self.visualize and modality_name == self.ego_modality:
                        lidar_np, object_bbx_center, object_bbx_mask = \
                        self.augment(lidar_np, object_bbx_center, object_bbx_mask) # only augment ego vehicle (student) for clip loss
                    if sensor_type == "lidar":
                        processed_lidar = eval(f"self.pre_processor_{modality_name}").preprocess(lidar_np)
                        selected_cav_processed.update({f'processed_features_{modality_name}': processed_lidar})
                    
                if self.visualize and modality_name == self.ego_modality:
                    selected_cav_processed.update({'origin_lidar': lidar_np}) # for visualization
            ...
            selected_cav_processed.update(
                {
                    ...
                    "object_bbx_mask": object_bbx_mask,
                    "object_ids": object_ids,
                    "modality_name": self.modality_name_list # 2 modalities of this cav for clip loss
                }
            )
