...
def visualize_single_sample_output_gt(pred_box_tensor,
    ...
    def custom_draw_geometry(pcd, pred, gt, new_origin=None):
    ...
    if show_vis:
        custom_draw_geometry(pcds, oabbs_pred, oabbs_gt, new_origin=[0, 0, 0])
    if save_path:
...
def save_o3d_visualization(element, save_path):
    """
    Save the visualization to folder using matplotlib.
    ...
    for box in element[1:]:
        if isinstance(box, o3d.geometry.LineSet):
            points = np.asarray(box.points)
            lines = np.asarray(box.lines)
            for line in lines:
                plt.plot(points[line, 0], points[line, 1], 'r-', linewidth=1)
