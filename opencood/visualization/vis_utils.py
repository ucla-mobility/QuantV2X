...
def visualize_feature_maps(output_dict, save_dir, step_id):
    """
    Visualize summed feature maps (over channels) from encoder and pyramid backbone.

    Args:
        output_dict: Dict from model forward
        save_dir: Directory to save visualizations
        step_id: Index of the sample
    """
    os.makedirs(save_dir, exist_ok=True)

    def plot_feature(tensor, tag, percentile=50):
        """
        tensor: (B, C, H, W) or (C, H, W)
        tag: name for the plot
        """
        print(tensor.shape)
        if tensor.dim() == 4:
            tensor = tensor[0]  # batch size 1 → (C, H, W)
        summed = tensor.sum(dim=0)  # (H, W)
        # summed = tensor.mean(dim=0)  # (H, W)

        plt.figure(figsize=(12, 10))
        feature_map = summed.detach().cpu().numpy()
        vmax = np.percentile(feature_map, percentile)  # we only see the minimum 30% of the values
        plt.imshow(feature_map, cmap='viridis', interpolation='none',vmin=20, vmax=80)
        plt.title(f"{tag} | Summed Feature Map")
        plt.colorbar()
        plt.tight_layout()
        save_path = os.path.join(save_dir, f"{tag}_step{step_id}.png")
        plt.savefig(save_path)
        plt.close()

    # Encoder features
    # encoder_feats = output_dict['feat_map_encoder']
    # plot_feature(encoder_feats, "encoder", percentile=100)

    # Fused feature
    fused_tensor = output_dict.get('feat_map_fusion', None)
    if fused_tensor is not None:
        plot_feature(fused_tensor, "fused_pyramid", percentile=100)


def visualize_feature_distribution(output_dict, save_dir, step_id):
    """
    Visualize distribution of feature map values from encoder and pyramid backbone.
    ...
    def plot_distribution(tensor, tag):
        """
        tensor: (B, C, H, W) or (C, H, W)
        tag: name for the plot
        """
        ...
        # Filter the values within the range of 20 to 80
        filtered_values = feature_map[(feature_map >= 20) & (feature_map <= 50)]

        # Generate a KDE curve for the distribution
        ...
        plt.title(f"{tag} | Feature Map Value Distribution (20-80)")
        plt.xlabel('Feature Value')
        plt.ylabel('Density')
        plt.grid(True, linestyle='--', alpha=0.5)

        save_path = os.path.join(save_dir, f"{tag}_distribution_step{step_id}.png")
        plt.savefig(save_path)
        plt.close()

    def save_distribution(tensor, tag):
        """
        tensor: (B, C, H, W) or (C, H, W)
        tag: name for the plot
        """
        ...
        # Filter the values within the range of 20 to 50
        filtered_values = feature_map[(feature_map >= 20) & (feature_map <= 50)]
        ...
        # Save to file
        save_path = os.path.join(save_dir, f"{tag}_distribution_step{step_id}.npz")
    ...
    fused_tensor = output_dict.get('feat_map_fusion', None)
    if fused_tensor is not None:
        plot_distribution(fused_tensor, "fused_pyramid")
        save_distribution(fused_tensor, "fused_pyramid")
