...
from opencood.models.stamp_modules.feature_alignnet_modules import (
    SCAligner,
    Res1x1Aligner,
    Res3x3Aligner,
    Res3x3Aligner,
...
try:
    ...
    class PositionalEncoding2D(nn.Module):
        ...
        def forward(self, tensor):
            if tensor.dim() != 4:
                raise ValueError("Expected (B, H, W, C) input for PositionalEncoding2D")
            b, h, w, c = tensor.shape
            channels = self.channels if self.channels is not None else c
            device = tensor.device
            dtype = tensor.dtype
            half = channels // 2
            pe = torch.zeros(h, w, channels, device=device, dtype=dtype)
            if half == 0:
                return pe.unsqueeze(0).repeat(b, 1, 1, 1)
            div_term = torch.exp(
                torch.arange(0, half, 2, device=device, dtype=dtype) * (-math.log(10000.0) / half)
            )
            pos_y = torch.arange(h, device=device, dtype=dtype).unsqueeze(1)
            pos_x = torch.arange(w, device=device, dtype=dtype).unsqueeze(1)
            pe_y = torch.zeros(h, half, device=device, dtype=dtype)
            pe_x = torch.zeros(w, half, device=device, dtype=dtype)
            ...
            pe[:, :, :half] = pe_y[:, None, :]
            pe[:, :, half : half + pe_x.shape[1]] = pe_x[None, :, :]
            return pe.unsqueeze(0).repeat(b, 1, 1, 1)

    class PositionalEncodingPermute2D(nn.Module):
        def __init__(self, channels):
            super().__init__()
            self.pe = PositionalEncoding2D(channels)

        def forward(self, tensor):
            if tensor.dim() != 4:
                raise ValueError("Expected (B, C, H, W) input for PositionalEncodingPermute2D")
            b, c, h, w = tensor.shape
            pe = self.pe(torch.zeros((b, h, w, c), device=tensor.device, dtype=tensor.dtype))
