...
def all_pair_l2(A, B):
    ...
    squared_dist = np.sum(A * A, 1, keepdims=True).repeat(TwoAB.shape[1], axis=1) \
            + np.sum(B * B, 1, keepdims=True).T.repeat(TwoAB.shape[0], axis=0) \
            - TwoAB
    squared_dist = np.maximum(squared_dist, 0.0)  # Ensure non-negative
    C = np.sqrt(squared_dist)
    return C


def cluster_alignment_relative_sample_np(
            pred_clusters_list,
            noisy_lidar_pose,
            proj2local_matrix,
            landmark_SE2=False,
            ...
            thres = 1.5,
            proj_first=True):
    """Perform cluster center for one sample.
    Correcting the relative pose.

    Args:
        pred_clusters_list : in world coordinate
            [[N_1, 3], ..., [N_cav, 3]]

        noisy_lidar_pose : 
            [N_cav, 6] in degree

        landmark_SE2 (bool, optional): 
            for landmark use yaw. Defaults to False.
        
        abandon_hard_cases (bool, optional):
            if True, algorithm will just return original poses for hard cases
        
        max_iterations (int, optional):
            pose optimizaiton iters. Defaults to 1000
    
        proj_first (bool, optional): Defaults to True
    ...
    pred_clusters_world_list = \
        [clusters.detach() for (idx, clusters) in enumerate(pred_clusters_list) if len(clusters)!=0]
    # project from world to each agent
    pred_cluster_local_list = \
        [project_points_by_matrix(clusters, proj2local_matrix[idx]).detach() for (idx, clusters) in enumerate(pred_clusters_list) if len(clusters)!=0]
    pred_len = \
        [len(clusters) for clusters in pred_clusters_list]
    
    box_idx_to_agent = []
    for i in range(N):
        box_idx_to_agent += [i] * pred_len[i] 
    
    pred_center_world_cat = np.concatenate(pred_clusters_world_list, axis=0)  # [sum(pred_box), 3]
    pred_center_local_cat = np.concatenate(pred_cluster_local_list, axis=0)  # [sum(pred_box), 3]
    ...
    for box_idx in range(cum): 
        ...
        cluster_dict[cluster_id]['landmark'] = landmark  # [x, y]
        cluster_dict[cluster_id]['landmark_SE2'] = False
    ...
    for landmark_id in range(agent_num, vertex_num):
        ...
        for box_idx in cluster_dict[landmark_id]['box_idx']:
            ...
            e_pose = pred_center_local_cat[box_idx][[0,1]].astype(np.float64)
            if landmark_SE2:
...
def box_alignment_relative_sample_np(
    ...
    for box_idx in range(cum): 
        ...
        cluster_dict[cluster_id]['active'] = True
        # cluster_dict (dict): 
        #   'box_idx' (list[int]): matched bbox idx
        #   'box_center_world' (list[list(3,)]): bbox world center
        #   'box_yaq' (list[folat]): bbox yaw
        #   'box_yaw_varies' (bool): 'box_yaq' var > yaw_var_thres
        #   'active' (bool)
        #   'landmark' (): shape(nx2 / nx3)
        #   'landmark_SE2' (bool): whether adaptive landmark 


        ########### adaptive_landmark ##################
        if landmark_SE2:
            # for k-th object, its pose is sampled from multiple bounding boxes in the same box cluster
            # which is essentially the unique object detected by multiple agents
            # use the first box's pose as landmark
    ...
    for agent_id in range(agent_num):
        v_id = agent_id
        # notice lidar_pose use degree format, translate it to radians.
        pose_np = noisy_lidar_pose[agent_id, [0,1,4]]   # in 2d plant for x, y, and degree
        pose_np[2] = np.deg2rad(pose_np[2])  # from degrees to radians
        v_pose = g2o.SE2(pose_np)
