...
# Heter mc functionality added by Aiden Wong <aidenwong@ucla.edu>
...
class HeterModelEarlyMC(nn.Module):
    """
    ...
    def __init__(self, args):
        ...
        self.cls_head = nn.Conv2d(self.out_channels, self.anchor_number * self.num_class * self.num_class, kernel_size=1)
        self.reg_head = nn.Conv2d(self.out_channels, self.anchor_number * 7 * self.num_class, kernel_size=1)
        if self.use_dir:
            self.dir_head = nn.Conv2d(self.out_channels,
                                      self.anchor_number * self.dir_bins * self.num_class,
                                      kernel_size=1)
