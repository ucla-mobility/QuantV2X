...
class GenComm(nn.Module):
    ...
    def forward(self, spatial_features, conditions, record_len=None):
        spatial_features_split = regroup(spatial_features, record_len)
        spatial_features = []
        for i, batch_features in enumerate(spatial_features_split):
            x = batch_features[0].repeat(record_len[i], 1, 1, 1)
        ...
        if self.training:
            ...
            ego_feat = spatial_features[0].unsqueeze(0)
    ...
    def forward_single(self, features, conditions, record_len=None):
...
if __name__ == '__main__':
    ...
    # Initialize model
    model = GenComm(model_cfg, embed_dim=channels)
    model = model.to(device)
    
    # Create dummy data dictionary with required tensors
    spatial_features =  torch.randn(batch_size, channels, height, width).to(device)
    conditions = torch.randn(batch_size, 2, height, width).to(device)
    ...
    with torch.autograd.set_detect_anomaly(True):
        output_dict = model(spatial_features, conditions, record_len)
        print(f"Training output shape: {output_dict['pred_feature'].shape}")
