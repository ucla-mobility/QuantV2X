...
class Enhancer_block(nn.Module):
    ...
    def forward(self, x,):
        ...
        # x_windows = window_partition(x, window_size)
        # x_windows = x_windows.view(-1, window_size*window_size, C)
        q_windows = window_partition(q, window_size).view(-1, window_size*window_size, C)
        k_v_windows = window_partition(k_v, window_size).view(-1, window_size*window_size, C) 
        attn_windows = self.window_attn(q_windows, attn_kv = k_v_windows)
        attn_windows = attn_windows.view(-1, window_size, window_size, C)
        x = window_reverse(attn_windows, window_size, H, W)
        x = x.view(B, H*W, C)
        x = shortcut + self.drop_path(x)
...
class Enhancer(nn.Module):
    def __init__(self, C, win_size, num_heads):
        super().__init__()
        self.block_1 = Enhancer_block(C, [16,16], num_heads)
        self.block_2 = Enhancer_block(C, win_size, num_heads)
        self.block_3 = Enhancer_block(C, [4, 4], num_heads)

    
    def forward(self, x, affine_matrix=None):
        # ego_feat = x[0].unsqueeze(0)
        # cav_feat = x[1:]
        # k_v = ego_feat
        # q = cav_feat
        s = self.block_1(x)
