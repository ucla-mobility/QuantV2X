...
from opencood.quant.quant_block import QuantDownsampleConv
...
def block_reconstruction(model: QuantModel, fp_model: QuantModel, block: BaseQuantBlock, fp_block: BaseQuantBlock,
    ...
    cached_inps = get_init(model, block, cali_data, batch_size=batch_size, 
                                        input_prob=True, keep_gpu=keep_gpu) # will be a list of dicts for basebevbackbone
    ...
    set_act_quantize_params(block, cached_inps=cached_inps[:min(256, cached_inps.size(0) if isinstance(cached_inps, torch.Tensor) else len(cached_inps))])
    ...
    for i in range(iters):
        ...
        out_drop = block(drop_inp) # noisy input
        out_quant = block(cur_inp) # clean input
...
class LossFunction:
    ...
    def __call__(self, pred, tgt, output_qt, output_fp):
        """
        ...
        hetero_loss = 0
        misalignment_loss = 0
        if output_qt is not None and output_fp is not None:
            hetero_loss = self.hetero_loss(F.log_softmax(pred / self.T, dim=1), F.softmax(tgt / self.T, dim=1)) / self.lam
            misalignment_loss = lp_loss(output_qt, output_fp, p=self.p)
        ...
        total_loss = rec_loss + round_loss if hetero_loss == 0 else rec_loss + round_loss + hetero_loss + misalignment_loss
