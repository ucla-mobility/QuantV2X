...
include = False
def find_unquantized_module(model: torch.nn.Module, module_list: list = [], name_list: list = [], parent_name=""):
    """Store subsequent unquantized modules in a list with full hierarchical names"""
    global include
    for name, module in model.named_children():
        full_name = f"{parent_name}.{name}" if parent_name else name  # Store full path name
        ...
        if isinstance(module, (QuantModule, BaseQuantBlock)):
            ...
            find_unquantized_module(module, module_list, name_list, full_name)  # Pass down full name
...
def layer_reconstruction(model: QuantModel, fp_model: QuantModel, layer: QuantModule, fp_layer: QuantModule,
                        cali_data: list, batch_size: int = 1, iters: int = 20000, weight: float = 0.001,
    ...
    for i in range(iters):
        ...
        out_drop = layer(drop_inp) # noisy input
        out_quant = layer(cur_inp) # clean input
        output = out_quant
