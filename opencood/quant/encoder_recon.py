...
from .encoder_recon_utils import get_encoder_out, get_encoder_input
...
def set_act_quantize_params(module: Union[QuantModel, QuantModule, BaseQuantBlock],
                            cached_inps: Union[list, torch.Tensor], channel_sizes: list=None):
    ...
    if isinstance(cached_inps, torch.Tensor):
        with torch.no_grad():
            ...
            for i, ch in enumerate(channel_sizes):
                end_ch = start_ch + ch
                sliced_input = cached_inps[start_ch:end_ch, :, :]  # cached_inps is (C, H, W)
                module(sliced_input.cuda())
                start_ch = end_ch  # Update for next slice
    else:
...
def encoder_reconstruction(qt_model: QuantModel, fp_model: QuantModel, qt_block: BaseQuantBlock, fp_block: BaseQuantBlock,
    ...
    cached_inps, cached_inp_list, channel_sizes = get_encoder_input(qt_model, qt_block, cali_data, input_prob=True, keep_gpu=keep_gpu)
    cached_outs, cached_output, cur_syms = get_encoder_out(fp_model, 
                                                        ...
                                                        bn_lr=bn_lr, 
                                                        lamb=lamb_c)
    set_act_quantize_params(qt_block, cached_inps=cached_inps, channel_sizes=channel_sizes)
