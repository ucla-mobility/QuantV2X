...
from .quant_layer import QuantModule, lp_loss, QuantSpconvModule
from .quant_model import QuantModel
from .quant_block import BaseQuantBlock, specials_unquantized_names
from .adaptive_rounding import AdaRoundQuantizer
from .second_recon_utils import get_encoder_out, get_encoder_input
...
def set_act_quantize_params(module: Union[QuantModel, QuantModule, BaseQuantBlock],
    ...
    for t in module.modules():
        if isinstance(t, (QuantSpconvModule, QuantModule, BaseQuantBlock)) and hasattr(t, 'act_quantizer'):
    ...
    for i in range(0, len(cached_inps)):
        with torch.no_grad():
            module(cached_inps[i]) # SpconvTensor as input to this module
        
    torch.cuda.empty_cache()

    for t in module.modules():
        if isinstance(t, (QuantSpconvModule, QuantModule, BaseQuantBlock)) and \
                hasattr(t, 'act_quantizer'):
            t.act_quantizer.set_inited(True)


def second_reconstruction(qt_model: QuantModel, fp_model: QuantModel, qt_block: BaseQuantBlock, fp_block: BaseQuantBlock,
    ...
    """
    cached_inp_list = get_encoder_input(qt_model, qt_block, cali_data, input_prob=True, keep_gpu=keep_gpu)
    cached_outs, cached_output, cur_syms = get_encoder_out(fp_model, 
                                                        fp_block, 
                                                        cali_data,
                                                        input_prob=True, 
                                                        keep_gpu=keep_gpu, 
    ...
    set_act_quantize_params(qt_block, cached_inps=cached_inp_list)
    ...
    for name, module in qt_block.named_modules():
        if isinstance(module, QuantSpconvModule):
            ...
            if hasattr(module, 'act_quantizer') and module.act_quantizer.delta is not None:
    ...
    for i in range(iters):
        ...
        cur_inp = cached_inp_list[idx]
        cur_sym = cur_syms[idx]
        output_fp = cached_output[idx].to(device)
        cur_out = cached_outs[idx].to(device)
        if input_prob < 1.0:
            mixed_features = torch.where(
                torch.rand_like(cur_inp.features) < input_prob,
                cur_inp.features,
                cur_sym.features
            )

            drop_inp = cur_inp.replace_feature(mixed_features)
        ...
        out_drop = qt_block(drop_inp) # noisy input
        out_quant = qt_block(cur_inp) # clean input

        err = loss_func(out_drop.dense(), cur_out, None, None)
