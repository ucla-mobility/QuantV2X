...
def test_parser():
    parser = argparse.ArgumentParser(description="Codebook encode/decode inference")
...
def inference_intermediate_fusion_encdec(batch_data, model, dataset):
    """
    Model inference for intermediate fusion using encode/decode approach.
    ...
    model : opencood.object
        Model with encode/decode methods
    ...
    # Use the encode/decode forward pass
    output_dict['ego'] = model.forward_with_encdec(cav_content)
...
def main():
    ...
    # Override the model class to use the encode/decode version
    original_model_name = hypes['model']['core_method']
    if 'heter_pyramid_collab_codebook_mc' in original_model_name.lower():
        print(f"Overriding model to use encode/decode version")
    ...
    if opt.show_sequence:
        if not HAS_OPEN3D:
            print("Warning: open3d not available, disabling sequence visualization")
    ...
    with tqdm(total=len(data_loader)) as pbar:
        for i, batch_data in enumerate(data_loader):
            with torch.no_grad():
                ...
                if opt.fusion_method == 'late':
                    ...
                    # Use the encode/decode version for intermediate fusion
                    pred_box_tensor, pred_score, gt_box_tensor, gt_label_tensor = \
                        inference_intermediate_fusion_encdec(batch_data,
                                                            model,
                                                            opencood_dataset)
                else:
        ...
        if elapsed_time > 0:
            average_speed = pbar.n / elapsed_time
